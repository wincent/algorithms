...
async function main() {
	...
	if (TIMING) {
		...
		const delta = afterInsert - beforeInsert;

		console.log(
			`Time to insert ${points.size} points: ${(delta / 1000).toFixed(
				3
			)}s`
		);

		const beforeQuerying = Date.now();

		for (let i = 0; i < 1000; i++) {
			points.nearest(new Point2D(Math.random(), Math.random()));
		}

		const afterQuerying = Date.now();

		console.log(
			`Nearest neighbor calculations per second: ${
				1 / ((afterQuerying - beforeQuerying) / 1_000_000)
			}`
		);
	}
	...
	const highlight = (point: Point2D, color = 'rgba(128, 128, 128, 0.8)') => {
		canvas(
